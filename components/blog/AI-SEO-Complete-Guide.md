...
const SEOContent = () => (
  <div>
    <p>
      ...
      supporting <strong>PDF, DOCX, TXT</strong> and other formats, with
      processing speed <strong>less than 5 seconds</strong>.
    </p>
  </div>
);
```
    ...
    >
      ...
      <span className="text-sm">
        {isFresh
          ...
          : `Last updated ${formatDistanceToNow(lastModified, {
              addSuffix: true
            })}`}
      </span>
    </div>
        ...
        {mentions.map(mention => (
          <article key={mention.url} className="mention-card">
            ...
            >
              ...
              <time dateTime={mention.date.toISOString()}>
...
export function checkContentHealth(
  ...
  return content.map(item => {
    ...
    const priority =
      daysSinceUpdate > 180 ? "high" : daysSinceUpdate > 90 ? "medium" : "low";
