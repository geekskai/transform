...
| `yarn indexnow:dry`         | **预演模式**，输出配置与 URL 预览，不发 IndexNow 请求（仍会拉取 sitemap） |
| `yarn indexnow:verbose`     | **详细日志**，显示每个子 sitemap 的解析过程，适合排查问题                 |
| `yarn indexnow:dry-verbose` | 预演 + 详细日志（等同 `--dry-run --verbose`）                             |
| `yarn indexnow:help`        | 打印 CLI 帮助                                                             |
...
yarn indexnow --sitemap https://folioify.com/sitemap.xml --batch-size 500 --retry 0
